...
```{r, echo = FALSE}
knitr::opts_chunk$set(fig.path = "course_project_a_figure/")
set.seed(1)
library(ggplot2)
```
...
```{r}
ggplot(data.frame(means), aes(x = means)) + geom_histogram(aes(y=..density..), colour = "black", fill = "whites", binwidth = lambda) + geom_density(colour = "skyblue1", fill="skyblue1", alpha = .3) + geom_vline(aes(xintercept = sample_mean), linetype = "dashed", size = 2, colour = "red") + scale_x_continuous(breaks = c(1:10))
```
