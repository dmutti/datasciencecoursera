...
```{r}
knitr::opts_chunk$set(fig.path='figure/', fig.width = 9, fig.height = 6, dpi = 144)
...
```{r}
top_events <- as.factor(unique(c(head(as.character(noaa_economic_impact$type), 5), head(as.character(noaa_health_impact$type), 5))))

noaa_most_harmful <- noaa %>%
...
ggplot(noaa_most_harmful, aes(x = type, y = total, fill = type)) + geom_bar(stat = "identity") + scale_fill_brewer(palette = "Paired") + coord_flip() + scale_y_continuous(labels = comma) + xlab("Severe Weather Event") + ylab("Frequency") + ggtitle("Most Harmful Occurring Events") + guides(fill = FALSE)
```
...
Our data allow us to conclude that **FLOODS** are the most harmful weather events from an economical point of view, with losses surpassing US$ 150 billion dollars; from a public health perspective, **TORNADOES** are the most harmful events, causing a total of 5633 fatalities and 91346 injuries. The top 5 most harmful ocurring events are `r noaa_most_harmful$type`.